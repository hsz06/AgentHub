...
2. 聊天主界面布局实现（消息流、输入框）
3. 单聊模式完整流程：支持流式输出(Streaming)的打字机交互体验
4. 基础消息类型支持：文本、代码块、Markdown渲染
5. 上下文持久化与Token管理：实现智能对话截断、历史消息摘要与持久化存储

### 第三阶段：多Agent系统与适配器层（第5周）
1. Agent数据模型设计（头/名/能力，支持用户维度的 API Key 独立配置 BYOK）
2. 统一适配器层抽象定义，封装统一的报错处理与重试(Retry)降级机制
...
1. 富媒体消息卡片组件系统设计
2. 网页预览卡片（安全的 Sandbox Iframe 容器设计）
3. 代码Diff视图卡片（差异对比展示）
